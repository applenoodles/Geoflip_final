...
#
# 原本這裡是一個 class（含快取、自動縮小範圍重試）。現在只留一支函式。
# =====================================================================
...
from app.models import make_poi


# ---------------------------------------------------------------------
# 我們接受的地點種類。
# 每一組是 (大分類, 允許的小種類)。
# 小種類是 None 代表「任何小種類都接受」（只用在古蹟 historic，且要求有名字）。
# ---------------------------------------------------------------------
_TAG_PROFILE = [
...
# 沒有名字的地點，給一個中文的種類名稱當顯示（不要在地圖上顯示英文 tag）
_CN_TYPE_NAMES = {
...
# ---------------------------------------------------------------------
# 把 Overpass 回傳的一筆資料整理成我們的 POI 字典
# ---------------------------------------------------------------------
...
def _element_coords(element):
    """取出一筆資料的經緯度；point 直接有 lat/lon，面/線則放在 center 裡。"""
...
def _normalize(element):
    """把 Overpass 的一筆資料轉成我們的 POI 字典；不合用回 None。"""
...
def _too_close(poi, selected, min_spacing_m):
    """這個 poi 是否離「已選的某個點」太近（小於 min_spacing_m 公尺）？"""
    for sel in selected:
        # 把經緯度差換算成大概的公尺差（在小範圍內夠準）
...
def _build_query(center_lat, center_lon, radius_m, limit):
    ...
    for category, allowed in _TAG_PROFILE:
        if allowed is None:
            # 古蹟：任何 historic 且有 name
            lines.append(
                '  nwr["' + category + '"]["name"]'
                + "(around:" + str(rad) + "," + str(center_lat) + "," + str(center_lon) + ");"
            )
        else:
            alt = "|".join(sorted(allowed))   # 例如 cafe|restaurant|...
            lines.append(
                '  nwr["' + category + '"~"^(' + alt + ')$"]'
                + "(around:" + str(rad) + "," + str(center_lat) + "," + str(center_lon) + ");"
            )
...
def _sort_key(poi):
    """排序用：分數高的排前面；分數一樣就照 id 排（結果才穩定）。"""
    return (-poi["score"], poi["id"])
...
def fetch_board_pois(center_lat, center_lon, radius_m, min_spacing_m, limit=60):
    """抓中心點附近的 POI，整理後回傳一個 POI 清單。

    失敗（沒網路、逾時、回應壞掉…）→ raise Exception。
    """
    query = _build_query(center_lat, center_lon, radius_m, limit)
    url = OVERPASS_BASE_URL.rstrip("/") + "/api/interpreter"
    ...
    # 分數高的排前面（這樣間距過濾時高分點會優先被留下）
    pois = sorted(pois, key=_sort_key)

    # 把太靠近的點去掉，讓棋盤分散一點
    pois = _spread_filter(pois, min_spacing_m)
