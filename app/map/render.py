# =====================================================================
# 用 Folium 畫互動地圖，回傳一整頁 HTML（主頁面用 <iframe> 嵌進來）。
#
...
#
# 注意：這裡的 poi 是 Poi 物件，所以用 poi.name、poi.owner（不是 poi["name"]）；
#       但 route 是普通字典，所以用 route["coordinates_lonlat"]。
# 座標提醒：Folium 要 [緯度, 經度]；OSRM 給 [經度, 緯度]，所以要對調。
# =====================================================================
...
def _own_source_popup(poi, player_color):
    ...
    return (
        ...
        + 'color:white;border:0;border-radius:4px;cursor:pointer">選為起點</button>'
    )
...
def _latest_route_overlay(state):
    """回傳 (折線座標, 走廊邊框座標)，都是 Folium 要的 [緯度, 經度]。
    ...
    if not state.routes:
        return None
    route = state.routes[-1]               # route 是字典
    ...
    ring_latlon = []
    for x, y in poly_m.exterior.coords:    # 走廊邊框上每個公尺座標點
...
def render_map_html(state):
    ...
    opening = (not finished) and state.in_opening_phase(cur, GAME_OPENING_MOVES_PER_PLAYER)
    ...
    if overlay is not None:
        ...
        if buffer_ring:
            folium.Polygon(
                ...
                color=route_color, weight=1,
                fill=True, fill_color=route_color, fill_opacity=0.12,
            ).add_to(fmap)
    ...
    for poi in state.pois:
        ...
        if finished:
            ...
            popup_html = _opening_popup(poi) if poi.owner is None else _popup_header(poi)
        else:
            if poi.owner == cur:
