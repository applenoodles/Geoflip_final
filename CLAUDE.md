...
| Module | Role |
|---|---|
| `app/models.py` | Dataclasses + `score_poi()`. No I/O, no external imports. Defines `MoveKind = "opening"|"route"|"flip"|"pass"` and `OPENING_MOVES_PER_PLAYER`. |
...
1. **2 players, 12 turns total.** Even turns → P1, odd turns → P2.
2. **Opening phase**: each player's first `OPENING_MOVES_PER_PLAYER` (=2) moves are `move_kind="opening"`. Only a neutral target POI; no source, no OSRM call, no buffer, no flip. Each consumes one turn.
3. **Normal moves** (after opening): require both `source_poi_id` (own POI) and `target_poi_id` (neutral POI). Server calls OSRM walking route source→target. If `duration_s > 600` → invalid (no state change).
...
7. **End conditions**: `turn_index >= max_turns` (12) OR no neutral POIs left OR two consecutive passes.
...
run when a popup opens belongs in `_page_script()`, hooked into the
`MutationObserver` already set up there.
